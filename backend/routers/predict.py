...
from core.config import BATCH_LIMIT, DEFAULT_MODEL
...
def _run_one(data: ApplicantInput) -> PredictionResponse:
    ...
    prob, prediction = predict(feature_values, DEFAULT_MODEL)
    shap_values = get_shap_values(feature_values, DEFAULT_MODEL)
...
def batch_predict(applicants: list[ApplicantInput]):
    if len(applicants) > BATCH_LIMIT:
        raise HTTPException(status_code=400, detail=f"Batch limit is {BATCH_LIMIT}")
